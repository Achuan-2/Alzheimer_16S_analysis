...
data <- read.table("./data/table_family.tsv", header = TRUE, row.names = 1, sep = "\t")
...
# 精简物种注释
rownames(data) <- sapply(str_split(rownames(data), "__"), "[", 6)
# data <- data[, -ncol(data)]
dim(data)
# 把绝对丰度变为相对丰度
...
p
ggsave("plot/family.svg", plot=p,width = 20, height = 7, units = "cm")
