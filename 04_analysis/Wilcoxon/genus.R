...
library(tidyverse)
library(stringr)
data <- read.table("./data/table_genus.tsv", header = TRUE, row.names = 1, sep = "\t")
group <- read.table("./data/metadata.txt", header = TRUE, sep = "\t")
## 过滤注释有问题的特征（6+4=10）
data <- filter(data, !grepl(";__|uncultured", rownames(data)))

# 精简物种注释
rownames(data) <- sapply(str_split(rownames(data), "__"), "[", 7)
# data <- data[, -ncol(data)]
dim(data)
# 把绝对丰度变为相对丰度
...
write.table(as.data.frame(AD_order), "genus.txt", row.names = F)
# 给因子排序
...
p
ggsave("plot/genus.svg", plot=p,width = 20, height = 15, units = "cm")
